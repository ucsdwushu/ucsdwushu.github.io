...
const Member = ({ title, name, desc, img, useSmallFont }) => {
  let fontSize
  if (useSmallFont) fontSize = "text-sm"
  else fontSize = "text-base"

  return (
    <div
      className="inline-block text-3xl lg:text-4xl border-solid mb-3 w-full bg-black"
      style={{ borderWidth: "1px" }}
    >
      <div className="px-8 py-8 border-solid">
        <h1
          className="text-xl"
          style={{ fontVariationSettings: "'wdth' 125, 'wght' 500" }}
        >
          {title}
        </h1>
        <h1
